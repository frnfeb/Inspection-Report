...
from .libreoffice_runner import get_session, recalculate_and_export
...
def _warm_up_libreoffice() -> None:
    """
    Starts the shared LibreOffice instance immediately when the server
    boots, instead of waiting for the first /api/generate call. Without
    this, whoever sends the very first request after a deploy/restart
    pays the full LibreOffice cold-start cost (profile init + font cache
    indexing) on top of their own request's work -- on a small instance
    this is exactly the kind of delay that can trip a platform-level
    timeout and show up in the browser as a bare NetworkError.
    Best-effort: if this fails, the first real request will just retry
    the same startup logic itself (get_session() is idempotent).
    """
    import threading
    ...
    def _boot():
        try:
            ...
            pass  # first real request will retry and surface any real error
